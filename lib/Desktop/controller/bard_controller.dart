...
import 'package:flemini/Desktop/model/bard_model.dart';
import 'package:flemini/consts/data.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:http/http.dart' as http;

class DBardAIController extends GetxController {
  DBardAIController({required this.userId});
  RxList<DBardmodel> historyList = RxList<DBardmodel>([]);
  ...
  Future<void> sendPrompt({String? prompt, String? text}) async {
    ...
    if ((text?.trim().isNotEmpty ?? false)) {
      ...
      historyList.add(DBardmodel(system: "user", message: text));
      ...
      final response = await http.post(Uri.parse(baseUrl + dapiKey),
      ...
      historyList.add(DBardmodel(system: "model", message: bardReply));
      isLoading.value = false;
    }
  }
